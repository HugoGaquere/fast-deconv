...
overlap_region compute_overlap_region(int y_center, int x_center, int a_height, int a_width,
...
__global__ void subtract_component_kernel(float* residual, const float* psf, const float* per_chan,
                                          overlap_region ovr, float gain, int dirty_freq_stride,
                                          int psf_freq_stride)
{
...
__global__ void subtract_component_kernel(float* residual, const float* psf, overlap_region ovr,
                                          float gain)
{
  const int tix = blockIdx.x * blockDim.x + threadIdx.x;
  const int tiy = blockIdx.y * blockDim.y + threadIdx.y;
                              ...
                              std::pair<int, int> peak_coords, float gain)
{
  ...
  const auto ovr = compute_overlap_region(peak_coords.first, peak_coords.second, residual_nrows,
                                          residual_ncols, psf.extent(0), psf.extent(1));

  dim3 block(32, 8);
  dim3 grid(CEIL_DIV(ovr.w, block.x), CEIL_DIV(ovr.h, block.y));
  kernel::subtract_component_kernel<<<grid, block, 0, stream_res.cuda_stream>>>(
      residual.data_handle(), psf.data_handle(), ovr, gain);
}
                              ...
                              std::pair<int, int> peak_coords, float gain)
{
  ...
  const auto ovr = compute_overlap_region(peak_coords.first, peak_coords.second, residual_nrows,
                                          residual_ncols, psf_nrows, psf_ncols);

  dim3 block(32, 8, 1);
  dim3 grid(CEIL_DIV(ovr.w, block.x), CEIL_DIV(ovr.h, block.y), n_freq);
  kernel::subtract_component_kernel<<<grid, block, 0, stream_res.cuda_stream>>>(
      residual.data_handle(), psf.data_handle(), spectral_coeffs.data_handle(), ovr, gain,
