                                        ...
                                        const core::device_vect<float>& weights_freq, float gamma)
{
  ...
  for (int b = 0; b < n_batch; b++) {
    ...
    linalg::weighted_sum_async(stream_res, psf_ptr, weights_freq.data_handle(), wmean, n_ch,
                               psf_npix);
    // 2. Max reduction -> d_maxes[f]
    cub::DeviceReduce::Max(d_temp, temp_bytes, wmean, d_maxes + b, psf_npix,
                           stream_res.cuda_stream);
  }

  // Bulk copy all max values to host and compute gains
  std::vector<float> gains(n_batch);
  CHECK_CUDA(cudaMemcpyAsync(gains.data(), d_maxes, sizeof(float) * n_batch,cudaMemcpyDeviceToHost, stream_res.cuda_stream));
  stream_res.sync();
