                                 ...
                                 core::device_span3d<float> scales)
{
  ...
  kernel::make_scales_kernel_half<<<grid_dim, block_dim, 0, stream_res.cuda_stream>>>(
      sigmas.data_handle(), scale_nrow, scale_ncol_half, scale_ncol_full, n_scales,
      scales.data_handle());
}

void convolve_with_scales(const core::resources& resources,
                          ...
                          core::device_span3d<float> out_scaled_dirty)
{
  ...
  linalg::pad_ifftshift(dirty.data_handle(), dirty_padded, ctx.input_nrow, ctx.input_ncol,
                                ctx.padded_nrow, ctx.padded_ncol, ctx.padding_nrow,
  ...
  for (int i = 0; i < n_scales; i++) {
    kernel::multiply_batched_kernel<<<CEIL_DIV(freq_total, 256), 256, 0, cuda_stream>>>(
        dirty_freq, scales.data_handle() + i * freq_total, scaled_dirty_freq, freq_total, 1, norm);

    CUFFT_CALL(cufftExecC2R(ctx.plan_backward(), scaled_dirty_freq, scaled_dirty));

    linalg::fftshift_crop(scaled_dirty, out_scaled_dirty.data_handle() + i * npix,
                                  ctx.input_nrow, ctx.input_ncol, ctx.padded_nrow,
                                  ctx.padded_ncol, ctx.padding_nrow, ctx.padding_ncol, 1,
                                  cuda_stream);
  }

  // we want untouched mean dirty at scale 0
  CHECK_CUDA(cudaMemcpyAsync(out_scaled_dirty.data_handle(), dirty.data_handle(),
                             npix * sizeof(float), cudaMemcpyDeviceToDevice,
                    ...
                    const std::vector<int>& retired_scales)
{
  ...
  size_t temp_bytes = 0;
  CHECK_CUDA(cub::DeviceSegmentedReduce::ArgMax(nullptr, temp_bytes, scaled_dirty.data_handle(),
                                                d_peaks, n_scales, d_offsets, d_offsets + 1,
                                                cuda_stream));

  void* d_temp = resources.alloc_async(temp_bytes, stream_res);
  CHECK_CUDA(cub::DeviceSegmentedReduce::ArgMax(d_temp, temp_bytes, scaled_dirty.data_handle(),
                                                d_peaks, n_scales, d_offsets, d_offsets + 1,
                                                cuda_stream));
                              ...
                              core::device_span3d<float> out_conv2_mean)
{
  ...
  if (scale_idx == 0) {
    CHECK_CUDA(cudaMemcpyAsync(out_conv_psf.data_handle(), psfs.data_handle(),
                               sizeof(float) * n_facets * facet_stride, cudaMemcpyDeviceToDevice,
                               stream_res.cuda_stream));
  ...
  for (int f = 0; f < n_facets; f++) {
    ...
    linalg::pad_ifftshift_batched(src, padded_psf, ctx.input_nrow, ctx.input_ncol,
                                          ctx.padded_nrow, ctx.padded_ncol,
   ...
   // 3. Multiply by G and G^2
    ...
    linalg::fftshift_crop(padded_conv, dst_conv, ctx.input_nrow, ctx.input_ncol,
                                  ctx.padded_nrow, ctx.padded_ncol, ctx.padding_nrow,
                                  ctx.padding_ncol, n_freq, cuda_stream);

    // 7. fftshift + crop for conv2_psf -> temporary
    linalg::fftshift_crop(padded_conv2, conv2_cropped_ptr, ctx.input_nrow, ctx.input_ncol,
                                  ctx.padded_nrow, ctx.padded_ncol, ctx.padding_nrow,
                                  ctx.padding_ncol, n_freq, cuda_stream);
                                       ...
                                       int nch, int psf_npix, int scale_idx, float gamma)
{
  ...
  for (int f = 0; f < n_facets; f++) {
    ...
    linalg::weighted_sum_async(stream_res, conv_psfs + f * facet_stride, weights, wmean, nch,
                               psf_npix);
