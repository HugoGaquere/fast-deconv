/// Benchmark: run the WSCMS minor-cycle loop on synthetic data with fixed
/// stopping criteria, sweeping over a cartesian grid of WSCMS parameters.
///
/// Unlike example_wscms (which loads real dump_ref exports), this driver
...
///
/// For each config the timed region wraps only run_wscms_cycles. One
/// wscms::context is built per config and reused across every repetition
...
/// across run_wscms_cycles calls, so it is cleared before each run to keep the
/// work deterministic. The dirty image is reset from a pristine master copy
/// before each run because run_wscms_cycles mutates it in place.
///
...
///
/// Usage:
///   bench_wscms [--mode=cartesian|ofat]
...
// DDFacet-style scale extents in pixels (see scripts/wscms_sigmas_bias.md):
...
static std::vector<float> gen_dirty(int n_freq, int nrow, int ncol, uint64_t seed)
{
  ...
  for (int f = 0; f < n_freq; ++f) {
    ...
    for (std::size_t i = 0; i < npix; ++i)
      dirty[static_cast<std::size_t>(f) * npix + i] = mean[i] * scale;
  }
  return dirty;
}
...
static std::size_t cufft_work_bytes(int padded_nrow, int padded_ncol, int forward_batch,
                                    int backward_batch, int n_backward_plans)
{
...
//   (3) The peak working set of run_wscms_cycles inside the pool. Buffer sizes
//       are exact element counts mirroring the alloc_async calls in wscms.cu /
...
static std::size_t estimate_bytes(const bench_config& c)
{
  ...
  const wscms::params p = make_fixed_params(c);  // the exact params this config runs with
  ...
  const std::size_t phase_psf =
      3 * nf * psf_pad * F + 3 * nf * psf_freq * CX + nf * psf_npix * F + psf_freq * F;
...
static config_result run_config(const bench_config& c, const bench_options& opt,
                                 host_data_cache& cache)
{
  config_result out;
  out.cfg = c;
  ...
  const uint64_t dirty_seed =
      opt.seed ^ (static_cast<uint64_t>(c.n_freq) * 0x9E3779B97F4A7C15ull +
                  static_cast<uint64_t>(c.nrow) * 0xC2B2AE3D27D4EB4Full +
                  static_cast<uint64_t>(c.ncol) * 0x165667B19E3779F9ull);
  const std::vector<float>& h_psfs =
      cache_get(cache.psfs, std::make_tuple(c.n_facet, c.n_freq, c.psf_nrow, c.psf_ncol),
                [&] { return gen_psfs(c.n_facet, c.n_freq, c.psf_nrow, c.psf_ncol); });
  const std::vector<int>& h_map =
      cache_get(cache.map_facet, std::make_tuple(c.nrow, c.ncol, c.n_facet),
                [&] { return gen_map_pixel_facet(c.nrow, c.ncol, c.n_facet); });
  const std::vector<float>& h_dirty =
      cache_get(cache.dirty, std::make_tuple(c.n_freq, c.nrow, c.ncol),
                [&] { return gen_dirty(c.n_freq, c.nrow, c.ncol, dirty_seed); });
  const std::vector<float>& h_jones =
      cache_get(cache.jones, std::make_tuple(c.n_freq, c.nrow, c.ncol),
                [&] { return std::vector<float>(static_cast<std::size_t>(c.n_freq) * npix, 1.0f); });
  const std::vector<unsigned char>& h_mask =
      cache_get(cache.mask, std::make_tuple(c.nrow, c.ncol),
                [&] { return std::vector<unsigned char>(npix, 0); });
  ...
  const wscms::params params = make_fixed_params(c);
  ...
  wscms::context ctx(opt.device, raw_psfs, xdes, mask, scale_sigmas, scale_bias, map_pixel_facet,
                     c.nrow, c.ncol, c.n_freq, fft_padding);

  const int total_runs = opt.warmup + opt.runs;
  for (int r = 0; r < total_runs; ++r) {
    // Reset dirty from the pristine master (run_wscms_cycles mutates it).
    ...
    wscms::wscms_result res = wscms::run_wscms_cycles(ctx, params, dirty, jones_norm, weights_freq);
...
static double ms_per_iter(const config_result& r)
{
  return r.total_iters > 0 ? r.stats.mean_ms / r.total_iters : 0.0;
}
...
static void write_csv_row(std::ostream& os, const config_result& r, const bench_options& opt)
{
  ...
  os << c.nrow << ',' << c.ncol << ',' << c.n_freq << ',' << c.n_scales << ',' << c.n_facet << ','
     << c.n_order << ',' << c.psf_nrow << ',' << c.psf_ncol << ',' << c.K << ',' << c.M << ','
     << r.total_iters << ',' << r.n_components << ',' << opt.runs << ',' << opt.warmup << ','
     << r.stats.mean_ms << ',' << r.stats.std_ms << ',' << r.stats.min_ms << ',' << r.stats.max_ms
     << ',' << r.stats.median_ms << ',' << iters_per_sec(r) << ',' << ms_per_iter(r) << ','
...
int main(int argc, char** argv)
{
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (a.rfind("--sizes=", 0) == 0) opt.sizes = parse_int_list(val("--sizes="));
    else if (a.rfind("--nfreq=", 0) == 0) opt.nfreqs = parse_int_list(val("--nfreq="));
    else if (a.rfind("--nscales=", 0) == 0) opt.nscales = parse_int_list(val("--nscales="));
    else if (a.rfind("--nfacet=", 0) == 0) opt.nfacets = parse_int_list(val("--nfacet="));
    else if (a.rfind("--norder=", 0) == 0) opt.norders = parse_int_list(val("--norder="));
    else if (a.rfind("--psf-frac=", 0) == 0) opt.psf_fracs = parse_double_list(val("--psf-frac="));
    else if (a.rfind("--K=", 0) == 0) opt.Ks = parse_int_list(val("--K="));
    else if (a.rfind("--M=", 0) == 0) opt.Ms = parse_int_list(val("--M="));
    else if (a.rfind("--runs=", 0) == 0) opt.runs = std::atoi(val("--runs=").c_str());
    ...
    else if (a.rfind("--max-configs=", 0) == 0) opt.max_configs = std::atoi(val("--max-configs=").c_str());
    else if (a.rfind("--mode=", 0) == 0) opt.mode = val("--mode=");
  ...
  printf("Mode: %s | Total configs: %zu  (runs=%d, warmup=%d per config)\n", opt.mode.c_str(),
         configs.size(), opt.runs, opt.warmup);
  ...
  if (opt.dry_run) {
    printf("%-6s %-6s %-6s %-7s %-7s %-7s %-10s %-4s %-4s %-10s\n", "nrow", "ncol", "nfreq",
           "nscales", "nfacet", "norder", "psf", "K", "M", "est_mem_MB");
    for (const auto& c : configs)
      printf("%-6d %-6d %-6d %-7d %-7d %-7d %dx%-7d %-4d %-4d %-10.1f\n", c.nrow, c.ncol, c.n_freq,
             c.n_scales, c.n_facet, c.n_order, c.psf_nrow, c.psf_ncol, c.K, c.M,
             estimate_bytes(c) / (1024.0 * 1024.0));
    return 0;
  }
  ...
  printf("%-5s %-5s %-3s %-3s %-4s %-3s %-9s %-4s %-4s | %10s %9s %12s %8s  %s\n", "nrow", "ncol",
         "nf", "ns", "nfac", "no", "psf", "K", "M", "mean_ms", "std_ms", "iters/s", "ms/it",
         "status");

  for (std::size_t i = 0; i < configs.size(); ++i) {
    const config_result r = run_config(configs[i], opt, cache);
    printf("%-5d %-5d %-3d %-3d %-4d %-3d %4dx%-4d %-4d %-4d | %10.3f %9.3f %12.1f %8.4f  %s\n",
           r.cfg.nrow, r.cfg.ncol, r.cfg.n_freq, r.cfg.n_scales, r.cfg.n_facet, r.cfg.n_order,
           r.cfg.psf_nrow, r.cfg.psf_ncol, r.cfg.K, r.cfg.M, r.stats.mean_ms, r.stats.std_ms,
           iters_per_sec(r), ms_per_iter(r), r.status.c_str());
    fflush(stdout);
    if (csv.is_open()) { write_csv_row(csv, r, opt); csv.flush(); }
  }
