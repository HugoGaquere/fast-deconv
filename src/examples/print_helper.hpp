...
template <>
inline void format_element<cuFloatComplex>(std::ostream& os,
...
// -----------------------------------------------------------------------------
template <typename T>
inline void print_host_vector(const T* data,
                              std::size_t n,
                              int indent = 0) {
...
template <typename T>
inline void print_host_matrix(const T* data,
                              std::size_t rows,
                              std::size_t cols,
                              int indent = 0) {
    std::string pad(indent, ' ');
...
inline void print_device_vector(const T* d_ptr,
                                ...
                                int indent = 0) {
    if (!d_ptr) {
        throw std::invalid_argument("print_device_vector: d_ptr is null");
    }
    if (n == 0) {
        std::string pad(indent, ' ');
        std::cout << pad << "[]" << std::endl;
    ...
    cudaError_t err = cudaMemcpyAsync(
        host.data(), d_ptr, n * sizeof(T),
        cudaMemcpyDeviceToHost, stream
    );
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("print_device_vector: cudaMemcpyAsync failed: ") +
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("print_device_vector: cudaStreamSynchronize failed: ") +
            cudaGetErrorString(err));
    }
...
template <typename T>
inline void print_device_matrix(const T* d_ptr,
                                std::size_t rows,
                                std::size_t cols,
                                cudaStream_t stream = 0,
                                int indent = 0) {
    if (!d_ptr) {
        throw std::invalid_argument("print_device_matrix: d_ptr is null");
    }
    if (rows == 0 || cols == 0) {
        std::string pad(indent, ' ');
        std::cout << pad << "[]" << std::endl;
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("print_device_matrix: cudaMemcpyAsync failed: ") +
            cudaGetErrorString(err));
    }
    ...
    if (err != cudaSuccess) {
        throw std::runtime_error(
            std::string("print_device_matrix: cudaStreamSynchronize failed: ") +
            cudaGetErrorString(err));
    }
