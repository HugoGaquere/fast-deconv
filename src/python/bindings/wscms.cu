...
void bind_wscms(py::module_& m)
{
  ...
  py::class_<wscms::Wscms>(wscms_module, "Wscms")
      .def(py::init<const core::device_span6d<float>&, const core::device_span4d<float>&,
                    const core::device_span4d<float>&, const core::device_span2d<float>&,
                    const core::device_vect<float>&, const core::device_span2d<bool>&,
                    ...
                    bool, float, int>(),
           py::arg("psfs"), py::arg("psfs_2"), py::arg("jones_norm"), py::arg("xdes"),
           py::arg("weights_freq"), py::arg("scale_masks"), py::arg("scale_sigmas"),
           py::arg("scale_bias"), py::arg("map_pixel_facet"), py::arg("gains"),
           py::arg("dirty_nrows"), py::arg("dirty_ncols"), py::arg("peak_factor"),
           py::arg("clean_negative"), py::arg("fft_padding"), py::arg("exec_device") = 0,
           py::keep_alive<1, 2>(),   // psfs
           py::keep_alive<1, 3>(),   // psfs_2
           py::keep_alive<1, 4>(),   // jones_norm
           py::keep_alive<1, 5>(),   // xdes
           py::keep_alive<1, 6>(),   // weights_freq
           py::keep_alive<1, 7>(),   // scale_masks
           py::keep_alive<1, 8>(),   // scale_sigmas
           py::keep_alive<1, 9>(),   // scale_bias
           py::keep_alive<1, 10>(),  // map_pixel_facet
           py::keep_alive<1, 11>())  // gains
      .def("run", &wscms::Wscms::run, py::arg("dirty"), py::arg("mean_residual"),
           py::arg("max_iterations"), R"pbdoc(Run the WSCMS algorithm)pbdoc");
}
}  // namespace fast_deconv::python
