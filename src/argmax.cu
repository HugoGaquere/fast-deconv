...
std::pair<int, float> _argmax(float *data, bool *mask, size_t size) {
  ...
  TransformInputIterator<float, MaskingOpT, CountingInputIterator<int>>
  ...
  DeviceReduce::ArgMax(d_temp_storage, temp_storage_bytes, masked_iter,
                       d_output, size);
  ...
  cudaMemcpy(&h_output, d_output, sizeof(KeyValuePair<int, float>),
...
extern "C" std::pair<int, float> argmax(float *data, bool *mask, size_t size,
                                        bool use_abs) {
  if (use_abs)
    return _argmax<MaskingOpAbs>(data, mask, size);
