...
auto cufftdx_3d_convolution(ValueType* input, ValueType* output, cudaStream_t stream) {
    ...
    CHECK_CUDA(cudaMemcpy(output_host.data(), output, Batches * flat_fft_size_bytes, cudaMemcpyDeviceToHost));
    CHECK_CUDA(cudaDeviceSynchronize());
...
int conv_3d() {
    ...
    CHECK_CUDA(cudaMalloc(&input, batches * flat_fft_size_bytes));
    CHECK_CUDA(cudaMalloc(&output, batches * flat_fft_size_bytes));

    // Copy input to the device
    CHECK_CUDA(cudaMemcpy(input, host_input.data(), batches * flat_fft_size_bytes, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaDeviceSynchronize());
