...
void make_scales(const core::resources& resources, float* sigmas, int scale_x, int scale_y_half,
                 int scale_y_full, int n_scales, float* scales)
{
  ...
  make_scales_kernel_half<<<grid_dim, block_dim, 0, stream_res.cuda_stream>>>(
      sigmas, scale_x, scale_y_half, scale_y_full, n_scales, scales);
}
...
scale_selection_result scale_selection(const core::resources& resources, float* scaled_dirty,
                                     const bool* mask, const float* bias, int n_scales, int npix_x,
                                     int npix_y, bool do_abs, bool per_scale_mask)
{
  const auto& stream_res = resources.get_stream_resources();
...
scale_convole_ctx make_scale_convole_ctx(int npix_x, int npix_y, int n_scales, float padding)
{
  ...
  ctx.img_x = npix_x;
  ctx.img_y = npix_y;
  ctx.padding_x = npad_x;
  ctx.padding_y = npad_y;
  ctx.img_padded_x = npix_x + 2 * npad_x;
  ctx.img_padded_y = npix_y + 2 * npad_y;
  ctx.freq_x = ctx.img_padded_x;
  ctx.freq_y = ctx.img_padded_y / 2 + 1;
  ctx.n_batches = n_scales;

  CUFFT_CALL(cufftPlan2d(&ctx.plan_forward, ctx.img_padded_x, ctx.img_padded_y, CUFFT_R2C));

  std::array<int, 2> fft_size{ctx.img_padded_x, ctx.img_padded_y};
...
{
  const int img_padded_total = ctx.img_padded_x * ctx.img_padded_y;
  const int freq_total = ctx.freq_x * ctx.freq_y;
  ...
  make_scales(resources, sigmas, ctx.freq_x, ctx.freq_y, ctx.img_padded_y, n_scales, scales,
              cuda_stream);

  // Pad + ifftshift dirty image
  linalg::detail::pad_ifftshift(dirty, dirty_padded, ctx.img_x, ctx.img_y, ctx.img_padded_x,
                                ctx.img_padded_y, ctx.padding_x, ctx.padding_y, cuda_stream);
  ...
  linalg::detail::fftshift_crop(scaled_dirty, out_scaled_dirty, ctx.img_x, ctx.img_y,
                                ctx.img_padded_x, ctx.img_padded_y, ctx.padding_x, ctx.padding_y,
                                n_scales, cuda_stream);
