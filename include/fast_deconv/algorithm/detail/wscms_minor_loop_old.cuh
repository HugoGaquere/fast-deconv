...
__host__ __device__ inline roi_strided compute_roi(int y_center, int x_center, int a_height,
...
__host__ __device__ inline auto unravel_spatial_index(uint flat_index, uint plane_stride,
...
//                        Update Mask
// ================================================================== //
__global__ void update_mask_kernel(core::device_span4d<float> dirty, core::device_span4d<bool> mask,
...
void update_mask(core::device_span4d<float>& dirty, core::device_span4d<bool>& mask, bool do_abs,
                 float threshold, core::stream_resources& resources)
{
  dim3 block(16, 16, 1);
  dim3 grid(CEIL_DIV(mask.extent(0), block.x), CEIL_DIV(mask.extent(1), block.y),
            mask.extent(2) * mask.extent(3));
  update_mask_kernel<<<grid, block, 0, resources.cuda_stream>>>(dirty, mask, do_abs, threshold);
...
//                     Dirty - PSF
// ================================================================== //
__global__ void clean_dirty_kernel(float* __restrict__ dirty, const float* __restrict__ psf,
                                   const bool* __restrict__ mask,
                                   const float* __restrict__ xdes_pinv,
                                   const float* __restrict__ xdes,
                                   core::device_vect<float> sqrt_weights, roi_strided roi,
                                   float gain, uint n_freq, uint n_order, uint peak_y, uint peak_x,
                                   uint dirty_plane_stride, uint psf_plane_stride,
                                   ComponentEntry* entries, uint iter_idx, int scale_idx)
{
  ...
  for (uint o = tid; o < n_order; o += block_size) {
    float sum = 0.0f;
    for (uint f = 0; f < n_freq; f++) sum += xdes_pinv[o * n_freq + f] * wy[f];
  ...
  for (uint f = tid; f < n_freq; f += block_size) {
    float sum = 0.0f;
    for (uint o = 0; o < n_order; o++) sum += xdes[f * n_order + o] * compact_spec_coeffs[o];
...
__global__ void masked_axpy_naive_kernel(float* __restrict__ dirty, const float* __restrict__ psf,
                                         const bool* __restrict__ mask, roi_strided roi, float gain,
                                         uint dirty_plane_stride, uint psf_plane_stride)
{
  const int local_x = blockIdx.x * blockDim.x + threadIdx.x;
  const int local_y = blockIdx.y * blockDim.y + threadIdx.y;
                       ...
                       uint psf_plane_stride, cudaStream_t stream)
{
  ...
  masked_axpy_naive_kernel<<<grid, block, 0, stream>>>(dirty_ptr, psf_slice_ptr, mask_ptr, roi,
                                                       gain, dirty_plane_stride, psf_plane_stride);
  CHECK_LAST_CUDA_ERROR();
}

void clean_dirty(float* dirty_ptr, const float* psf_slice_ptr, const bool* mask_ptr,
                 const float* xdes_pinv_ptr, const float* xdes_ptr,
                 core::device_vect<float> sqrt_weights, roi_strided roi, float gain, uint n_freq,
                 uint n_order, uint n_freq_pol, uint peak_y, uint peak_x, uint dirty_plane_stride,
                 uint psf_plane_stride, ComponentEntry* entries, uint iter_idx, int scale_idx,
                 cudaStream_t stream)
{
  dim3 block(16, 16, 1);
  dim3 grid(CEIL_DIV(roi.w, block.x), CEIL_DIV(roi.h, block.y), n_freq_pol);
  const uint smem_size = (2 * n_freq + n_order) * sizeof(float);
  clean_dirty_kernel<<<grid, block, smem_size, stream>>>(
      dirty_ptr, psf_slice_ptr, mask_ptr, xdes_pinv_ptr, xdes_ptr, sqrt_weights, roi, gain, n_freq,
      n_order, peak_y, peak_x, dirty_plane_stride, psf_plane_stride, entries, iter_idx, scale_idx);
  CHECK_LAST_CUDA_ERROR();
}
...
// {
  ...
  // CHECK_CUDA(cudaMallocAsync(reinterpret_cast<void**>(&d_Xdes_pinv_ptr),
  //                            n_order * n_freq * sizeof(float), stream_1));
  ...
  // CHECK_CUDA(cudaMallocAsync(reinterpret_cast<void**>(&dirty_spectral_peak_values),
  //                            n_freq * sizeof(float), stream_2));
  //
  // float* peak_value;
  // uint* peak_idx;
  ...
  //   const roi_strided roi =
  //       compute_roi(peak_y, peak_x, dirty_height, dirty_width, psf_height, psf_width);
  ...
  //   const float* psf_slice =
  //       psfs.data_handle() + scale_idx * psf_scale_stride + facet_idx * psf_facet_stride;
  //
  //   clean_dirty(dirty.data_handle(), psf_slice, mask.data_handle(), d_Xdes_pinv_ptr,
  //               ctx.Xdes.data_handle(), ctx.sqrt_weights, roi, gain, n_freq, n_order, n_freq_pol,
  //               peak_y, peak_x, dirty_plane_stride, psf_plane_stride, d_entries, n_iter,
  //               static_cast<int>(scale_idx), stream_1);
  //
  //   masked_axpy_naive(scaled_dirty.data_handle(), psf_slice, mask.data_handle(), roi, gain,
  //                     n_freq_pol, dirty_plane_stride, psf_plane_stride, stream_2);
  //
  //   matrix::argmax_async(peak_value, peak_idx, scaled_dirty.data_handle(), mask.data_handle(),
