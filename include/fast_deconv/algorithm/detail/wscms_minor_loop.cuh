...
__host__ __device__ inline roi_strided compute_roi(
  int x_center, int y_center, int a_width, int a_height, int b_width, int b_height)
{
  // X axis
                                   ...
                                   int scale_idx)
{
  ...
  if (mask[a_offset])
    dirty[a_offset] -= psf[b_offset] * gain * per_chan_spec_coeffs[fp];
}
                                         ...
                                         uint psf_plane_stride)
{
  ...
  if (mask[a_offset])
    dirty[a_offset] -= psf[b_offset] * gain;
}
                       ...
                       cudaStream_t stream)
{
  ...
  masked_axpy_naive_kernel<<<grid, block, 0, stream>>>(
    dirty_ptr, psf_slice_ptr, mask_ptr, roi, gain,
    dirty_plane_stride, psf_plane_stride);
  CHECK_LAST_CUDA_ERROR();
}
                   ...
                   cudaStream_t stream)
{
  ...
  clean_dirty_kernel<<<grid, block, smem_size, stream>>>(
    dirty_ptr, psf_slice_ptr, mask_ptr, xdes_pinv_ptr, xdes_ptr, sqrt_weights,
    roi, gain, n_freq, n_order, peak_y, peak_x,
    dirty_plane_stride, psf_plane_stride, entries, iter_idx, scale_idx);
  CHECK_LAST_CUDA_ERROR();
}
                       ...
                       core::stream_resources& resources)
{
  ...
  CHECK_CUDA(cudaMallocAsync( reinterpret_cast<void**>(&d_Xdes_pinv_ptr), n_order * n_freq * sizeof(float), stream_1));
  ...
  CHECK_CUDA(cudaMallocManaged(reinterpret_cast<void**>(&d_entries), ctx.n_subminor_iter * sizeof(ComponentEntry)));

  float* dirty_spectral_peak_values;
  CHECK_CUDA(cudaMallocAsync(reinterpret_cast<void**>(&dirty_spectral_peak_values), n_freq * sizeof(float), stream_2));
  ...
  // Sync before reading managed memory on host
  resources_1.sync();
  ...
  while (*peak_value > threshold && n_iter < ctx.n_subminor_iter) {
    // Extract spatial (y, x) from 4D flat index: flat = ch*(npol*h*w) + pol*(h*w) + y*w + x
    // Since h*w divides evenly into the higher dims, flat % (h*w) gives spatial index
    const uint spatial_idx = *peak_idx % dirty_plane_stride;
    const uint peak_y = spatial_idx / dirty_width;
    const uint peak_x = spatial_idx % dirty_width;
    const roi_strided roi = compute_roi(peak_x, peak_y, dirty_width, dirty_height, psf_width, psf_height);
    ...
    clean_dirty(dirty.data_handle(), psf_slice, mask.data_handle(),
                  d_Xdes_pinv_ptr, ctx.Xdes.data_handle(), ctx.sqrt_weights, roi, gain,
                  n_freq, n_order, n_freq_pol, peak_y, peak_x,
                  dirty_plane_stride, psf_plane_stride,
                  d_entries, n_iter, static_cast<int>(scale_idx), stream_1);
