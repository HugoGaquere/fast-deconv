...
#include <fast_deconv/algorithm/detail/clean_minor_loop.cuh>
...
void wscms_minor_cycles(const core::resources& resources, core::device_span2d<float>& residual,
                        ...
                        int max_iter)
{
  if (residual.extent(0) != map_pixels_facets.extent(0) || residual.extent(1) != map_pixels_facets.extent(1))
                                  ...
                                  int max_iter)
{
  ...
  detail::wscms_minor_cycles_host_loop(resources, residual, psfs_2, map_pixels_facets, gains,
...
{
  if (static_cast<int>(dirty.extent(0)) != ctx.img_nrow || static_cast<int>(dirty.extent(1)) != ctx.img_ncol)
  ...
  if (static_cast<int>(scales.extent(1)) != ctx.freq_nrow || static_cast<int>(scales.extent(2)) != ctx.freq_ncol)
  ...
  if (static_cast<int>(out_scaled_dirty.extent(0)) != ctx.n_batches)
    throw std::invalid_argument( "scale_convolve: out_scaled_dirty n_scales must match ctx.n_batches");
  if (static_cast<int>(out_scaled_dirty.extent(1)) != ctx.img_nrow || static_cast<int>(out_scaled_dirty.extent(2)) != ctx.img_ncol)
    throw std::invalid_argument( "scale_convolve: out_scaled_dirty spatial dimensions must match ctx image size");
