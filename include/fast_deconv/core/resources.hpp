...
class stream_resources {
 public:
  stream_resources(const cudaMemPool_t& mem_pool, uint flag) : mem_pool_(mem_pool)
  {
  ...
  T* alloc_async(uint64_t n) const
  {
    ...
    uint64_t num_bytes;
    if constexpr (std::is_void_v<T>) num_bytes = n;
    else num_bytes = n * sizeof(T);
...
class stream_resources_pool {
 public:
  ...
  explicit stream_resources_pool(const cudaMemPool_t& mem_pool, uint8_t pool_size = default_size,
                                 uint8_t flag = default_flag)
  {
    for (uint8_t i = 0; i < pool_size; i++) streams_.push_back(std::make_unique<stream_resources>(mem_pool, flag));
  }
...
class resources {
 public:
  resources(uint8_t device) : device_(device), stream_res_pool_(memory_pool_)
  {
