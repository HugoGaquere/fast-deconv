                         ...
                         core::stream_resources& resources)
{
  if (use_abs)
    detail::argmax_async<detail::masking_op_abs>(
      resources, data, mask, size, mask_size, d_max_out, d_index_out);
  else
    detail::argmax_async<detail::masking_op>(
      resources, data, mask, size, mask_size, d_max_out, d_index_out);
}

inline std::pair<int, float> argmax(const float* data,
                                    const bool* mask,
                                    size_t size,
                                    size_t mask_size,
                                    bool use_abs,
                                    ...
                                    core::stream_resources& resources)
{
  ...
  return argmax(data.data_handle(), mask.data_handle(), data.size(), mask.size(), use_abs, resources);
}
