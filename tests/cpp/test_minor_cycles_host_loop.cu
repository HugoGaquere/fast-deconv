...
class MinorCyclesHostLoop : public ::testing::Test {
 protected:
  static fs::path data_dir()
  {
    return fs::path(__FILE__).parent_path().parent_path() / "data" / "minor_cyles_reference_data";
  }
...
TEST_F(ScaleSelectionTest, ScaleSelectionResult)
{
  ...
  fast_deconv::core::resources resources(0);
  const fast_deconv::core::stream_resources& stream_res = resources.get_stream_resources();
  ...
  EXPECT_EQ(result.best_row, expected_xy.as_int32()[0]);
  EXPECT_EQ(result.best_col, expected_xy.as_int32()[1]);
  EXPECT_NEAR(result.best_peak, expected_peak.as_float32()[0], 1e-8f);
...
TEST_F(ScaleSelectionTest, CopyScaleSlice)
{
  ...
  resources.free_async(d_dst, stream_res);
  stream_res.sync();
}
